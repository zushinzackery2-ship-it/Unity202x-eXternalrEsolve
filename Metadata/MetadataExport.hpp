...
#include "MetadataPEParser.hpp"
#include "MetadataHeader.hpp"
...
inline std::vector<std::uint8_t> ExportMetadataImpl(std::uint32_t requiredVersion)
{
    std::vector<std::uint8_t> result;
...
{
    return detail_metadata::ExportMetadataImpl(0);
}
...
inline std::vector<std::uint8_t> ExportMetadataTVersion()
{
    ...
    return detail_metadata::ExportMetadataImpl(version);
}
