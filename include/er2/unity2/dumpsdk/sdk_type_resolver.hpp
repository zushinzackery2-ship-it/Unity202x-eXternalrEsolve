...
private:
    std::string DescribeTypePtr(std::uintptr_t typePtr, int depth)
    {
        ...
        if (base.empty())
        {
            ...
            {
                // For CLASS (0x12) and VALUETYPE (0x11), dataPtr could be:
                // 1. A pointer to TypeDefinition in metadata (Unity 6+)
                // 2. A TypeDefinitionIndex directly (older versions)
                
                // First try: check if dataPtr is a pointer to metadata
                ...
                // Second try: treat dataPtr as TypeDefinitionIndex directly
                if (base.empty())
                {
                    const std::uint32_t typeDefIndex = static_cast<std::uint32_t>(dataPtr);
                    const std::uint32_t typeDefCount = header_.typeDefinitionsSize / static_cast<std::uint32_t>(sizeof(DumpSdk6TypeDefRaw));
                    
                    if (typeDefIndex < typeDefCount)
                    {
                        const std::size_t off = static_cast<std::size_t>(header_.typeDefinitionsOffset) + static_cast<std::size_t>(typeDefIndex) * sizeof(DumpSdk6TypeDefRaw);
                        if (off + sizeof(DumpSdk6TypeDefRaw) <= meta_.size())
                        {
                ...
                // Third try: read from dataPtr as Il2CppClass pointer
