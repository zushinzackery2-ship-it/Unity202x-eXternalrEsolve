...

{
    ...
    // header[0x28..] Il2CppGlobalMetadataHeader::propertiesOffset/propertiesSize

    std::uint32_t propertiesOffset = 0;
...

{
    ...
    std::int32_t propertiesOffsetI = 0;

    std::int32_t propertiesSizeI = 0;
    ...
    if (!detail_metadata_header::ReadI32FromMem(mem, metaBase + 0x28u, propertiesOffsetI)) return false;

    if (!detail_metadata_header::ReadI32FromMem(mem, metaBase + 0x2Cu, propertiesSizeI)) return false;
    ...
    out.propertiesOffset = detail_metadata_header::ToPositiveU32(propertiesOffsetI);

    out.propertiesSize = detail_metadata_header::ToPositiveU32(propertiesSizeI);
...

{
    ...
    const std::int32_t propertiesOffsetI = detail_metadata_header::ReadI32LE(b + 0x28u);

    const std::int32_t propertiesSizeI = detail_metadata_header::ReadI32LE(b + 0x2Cu);
