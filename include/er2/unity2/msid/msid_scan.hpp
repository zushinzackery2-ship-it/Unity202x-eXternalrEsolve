...
inline bool ValidateUnityObject(const IMemoryAccessor& mem, std::uintptr_t obj, const GomOffsets& gomOff)
{
    if (!IsCanonicalUserPtr(obj)) return false;
    std::uintptr_t managed = 0;
    if (!GetManagedFromNative(mem, obj, gomOff, managed)) return true;
    if (!IsCanonicalUserPtr(managed)) return false;
    constexpr std::size_t kManagedNativeOffset = 0x10;
    std::uintptr_t native = 0;
    if (!ReadPtr(mem, managed + kManagedNativeOffset, native)) return false;
    return native == obj;
}
...
{
    if (!IsCanonicalUserPtr(entriesBase)) return 0;
    if (expectedCount == 0 || expectedCount > 5000000) return 0;
    if (capacity == 0 || capacity > 20000000) return 0;
    ...
    for (std::uint32_t batchStart = 0; batchStart < capacity; batchStart += kBatchSize)
    {
        ...
        if (!mem.Read(batchAddr, batch.data(), batchSize)) continue;
        ...
        for (std::uint32_t i = 0; i < batchCount; ++i)
        {
            ...
            if (hashMask == 0xFFFFFFFFu || hashMask == 0xFFFFFFFEu) continue;
            std::uintptr_t obj = 0;
            std::memcpy(&obj, batch.data() + entryOff + static_cast<std::size_t>(kMsIdEntryObject), sizeof(obj));
            if (ValidateUnityObject(mem, obj, gomOff)) ++objCount;
        }
        if (objCount >= expectedCount) break;
    }
    return objCount;
}
...
inline bool FindMsIdToPointerSlotVaByScan(
    ...
    std::uint32_t* outBestObjCount,
    std::uintptr_t gomSlotVa)
{
    outMsIdToPointerSlotVa = 0;
    if (outBestObjCount) *outBestObjCount = 0;

    std::uint32_t sizeOfImage = 0;
    std::vector<ModuleSection> sections;
    if (!ReadModuleSections(mem, unityPlayer.base, sizeOfImage, sections)) return false;

    constexpr std::uintptr_t kMaxDistanceFromGom = 8000;
    std::uintptr_t searchStart = 0;
    std::uintptr_t searchEnd = 0;
    if (gomSlotVa)
    {
        searchStart = (gomSlotVa > kMaxDistanceFromGom) ? (gomSlotVa - kMaxDistanceFromGom) : 0;
        searchEnd = gomSlotVa + kMaxDistanceFromGom;
    }
    ...
    for (const auto& s : sections)
    {
        ...
        const std::uintptr_t secEnd = secVa + s.size;
        if (gomSlotVa && (secEnd < searchStart || secVa > searchEnd)) continue;

        std::vector<std::uint8_t> buf;
        buf.resize(s.size);
        if (!mem.Read(secVa, buf.data(), buf.size())) continue;
        ...
        for (std::size_t i = 0; i + sizeof(std::uintptr_t) <= buf.size(); i += sizeof(std::uintptr_t))
        {
            ...
            if (gomSlotVa && (addr < searchStart || addr > searchEnd)) continue;

            std::uintptr_t ptr = 0;
            std::memcpy(&ptr, buf.data() + i, sizeof(ptr));
            if (!IsCanonicalUserPtr(ptr)) continue;

            std::uint8_t baseData[0x10] = {};
            if (!mem.Read(ptr, baseData, sizeof(baseData))) continue;

            std::uintptr_t entriesBase = 0;
            std::memcpy(&entriesBase, baseData + static_cast<std::size_t>(kMsIdSetEntriesBase), sizeof(entriesBase));
            if (!IsCanonicalUserPtr(entriesBase)) continue;
            if (!IsReadableByte(mem, entriesBase)) continue;
            ...
            if (count == 0 || count > 5000000) continue;
            if (capacity == 0 || capacity > 20000000) continue;
    ...
    if (!bestAddr) return false;
    outMsIdToPointerSlotVa = bestAddr;
    if (outBestObjCount) *outBestObjCount = bestObjCount;
    return true;
}
