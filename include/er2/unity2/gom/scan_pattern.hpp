...
inline bool IsWritableProtectForPointerScan(std::uint32_t protect)
{
    const std::uint32_t p = protect & 0xFFu;
...
inline bool FindProcessPrivateWritablePointerGlobal(
    const IMemoryAccessor& mem,
    HANDLE process,
    std::uintptr_t value,
    std::uintptr_t& outHit,
    std::size_t chunkSize,
    HitPredicate acceptHit)
{
    outHit = 0;
    if (!process || !value)
    {
    ...
    while (cursor < kUserMax)
    {
        ...
        if (regionSize < sizeof(std::uintptr_t))
        {
            continue;
        }
        ...
        if (!IsWritableProtectForPointerScan(static_cast<std::uint32_t>(mbi.Protect)))
        {
            continue;
        }

        for (std::uintptr_t readAddr = regionBase; readAddr < regionEnd;)
        {
            ...
            if (toRead < sizeof(std::uintptr_t))
            {
                break;
            }

            chunk.resize(toRead);
            if (mem.Read(readAddr, chunk.data(), toRead))
            {
                const std::size_t alignMask = sizeof(std::uintptr_t) - 1;
                const std::size_t first = static_cast<std::size_t>((sizeof(std::uintptr_t) - (readAddr & alignMask)) & alignMask);
                for (std::size_t i = first; i + sizeof(std::uintptr_t) <= toRead; i += sizeof(std::uintptr_t))
                {
                    std::uintptr_t cur = 0;
                    std::memcpy(&cur, chunk.data() + i, sizeof(cur));
                    if (cur != value)
                    {
                        continue;
                    }

                    const std::uintptr_t hit = readAddr + static_cast<std::uintptr_t>(i);
                    if (acceptHit(hit))
                    {
                        outHit = hit;
                        return true;
