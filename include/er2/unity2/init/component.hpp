...

{
    ...
    if (!er2::GetComponentPool(Mem(), gameObjectNative, g_ctx.gomOff, pool) || !pool) return result;



    std::int32_t count = 0;

    if (!er2::GetComponentCount(Mem(), gameObjectNative, g_ctx.gomOff, count) || count <= 0) return result;
